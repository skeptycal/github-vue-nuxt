  ...
  export default {
    ...
    methods: {
      getResultsIfLastPage (event) {
        if (theStore.getters.browseRepoList.length / event.rowsPerPage === event.page) {
      ...
      loadRepositories (sinceID, isConcat) {
        ...
        gitHubAPI.fetchRepositories(params, function (response) {
          // success
          nextSince = response.headers.nextSince
          if (isConcat) {
            let currentRepoList = theStore.getters.browseRepoList
            theStore.commit('updateBrowseRepoList', currentRepoList.concat(response.data))
            return
          }
