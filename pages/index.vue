<template>
  <div id="#app">
    <v-container>
      ...
      <v-layout>
        <v-flex
          v-if="resultsCountForTerm(false) === 0"
          xs6
          offset-xs5
          justify-center
          align-center
        ...
        <v-flex v-if="repoList.length > 0" xs12>
          <v-list two-line dense>
            <v-subheader class="result-count">
              &#8220;{{ currentSearchTerm }}&#8221; -
              <span v-if="!searchInProgress"
                >&nbsp; {{ resultsCountForTerm(true) }} Results</span
              >
      ...
      <v-layout>
        <v-flex v-if="isSearching" xs2 offset-xs5>
          <clip-loader :color="'#0d0d0d'" size="10px" class="spinner" />
        </v-flex>
      </v-layout>
    </v-container>
  </div>
...
export default {
  ...
  methods: {
    ...
    loadNextPage() {
      ...
      theStore.commit(
        "updateRequestedPage",
        theStore.getters.requestedPage + 1
      );
